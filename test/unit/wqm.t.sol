...
import { IynETH } from "test/mocks/IynETH.sol";

import { WithdrawalQueueManager, IWithdrawalQueueManager } from "../../src/WithdrawalQueueManager.sol";
import { ynETHRedemptionAssetsVault } from "test/mocks/ynETHRedemptionAssetsVault.sol";

import { MockRedeemableYnETH } from "test/mocks/MockRedeemableYnETH.sol";
...
contract ynETHWithdrawalQueueManagerTest is Test {
    ...
    function calculateNetEthAndFee(
        ...
        public
        view
        returns (uint256 netEthAmount, uint256 feeAmount)
    {
    ...
    function testRequestWithdrawal(uint256 _amount) public {
        ...
        assertEq(
            _withdrawalRequest.redemptionRateAtRequestTime,
            redemptionAssetsVault.redemptionRate(),
            "testRequestWithdrawal: E2"
        );
        ...
        assertEq(
            manager.pendingRequestedRedemptionAmount(),
            _pendingRequestedRedemptionAmountBefore + _amount,
            "testRequestWithdrawal: E6"
        );
    }
    ...
    function testRequestWithdrawalWithMaxUintAmount() public {
        ...
        vm.prank(user);
        vm.expectRevert(
            abi.encodeWithSelector(IERC20Errors.ERC20InsufficientBalance.selector, user, userBalance, maxUintAmount)
        );
    ...
    function testRequestWithdrawalWithInsufficientApproval() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IERC20Errors.ERC20InsufficientAllowance.selector, address(manager), approvedAmount, amount
            )
        );
        manager.requestWithdrawal(amount);
    }
    ...
    function testClaimWithdrawalSuccesfully(uint256 _amount) public {
        ...
        manager.claimWithdrawal(
            IWithdrawalQueueManager.WithdrawalClaim({ tokenId: tokenId, receiver: user, finalizationId: finalizationId })
        );
        ...
        assertApproxEqAbs(
            address(redemptionAssetsVault).balance, _vaultBalanceBefore - _amount, 1000, "testClaimWithdrawal: E8"
        );
    ...
    function testClaimWithdrawalRevertsWhenInsufficientVaultBalance() public {
        ...
        vm.prank(user);
        vm.expectRevert(
            abi.encodeWithSelector(WithdrawalQueueManager.InsufficientBalance.selector, insufficientAmount, amount)
        );
        manager.claimWithdrawal(
            IWithdrawalQueueManager.WithdrawalClaim({ tokenId: 0, receiver: user, finalizationId: finalizationId })
        );
    }
    ...
    function testClaimOneWithdrawalNotFinalized() public {
        ...
        vm.prank(user);
        manager.claimWithdrawal(
            IWithdrawalQueueManager.WithdrawalClaim({ tokenId: tokenId, receiver: user, finalizationId: 0 })
        );
    }
    ...
    function testClaimWithdrawalForNonExistentTokenId() public {
        ...
        vm.prank(user);
        vm.expectRevert(
            abi.encodeWithSelector(WithdrawalQueueManager.CallerNotOwnerNorApproved.selector, nonExistentTokenId, user)
        );

        manager.claimWithdrawal(
            IWithdrawalQueueManager.WithdrawalClaim({ tokenId: nonExistentTokenId, receiver: user, finalizationId: 0 })
        );
    }
    ...
    function testClaimWithdrawalForAlreadyProcessedWithdrawal() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(WithdrawalQueueManager.CallerNotOwnerNorApproved.selector, tokenId, user)
        );
        vm.prank(user);
        manager.claimWithdrawal(
            IWithdrawalQueueManager.WithdrawalClaim({ tokenId: tokenId, receiver: user, finalizationId: finalizationId })
    ...
    function testtestClaimWithdrawalNotOwner() public {
        ...
        vm.prank(notOwner);
        vm.expectRevert(
            abi.encodeWithSelector(WithdrawalQueueManager.CallerNotOwnerNorApproved.selector, tokenId, notOwner)
        );
        manager.claimWithdrawal(
            IWithdrawalQueueManager.WithdrawalClaim({ tokenId: tokenId, receiver: notOwner, finalizationId: 0 })
        );
    }
    ...
    function testclaimWithdrawals(uint256 _amount) public {
        ...
        assertApproxEqAbs(
            address(redemptionAssetsVault).balance, _vaultBalanceBefore - _amount, 1000, "testclaimWithdrawals: E8"
        );
    ...
    function testClaimWithdrawalsWithIncreasedRedemptionRate() public {
        ...
        assertEq(
            ...
            expectedNetEthAmount,
            "User should receive ETH based on initial redemption rate"
        );
    ...
    function testClaimWithdrawalsWithDecreasedRedemptionRate() public {
        ...
        uint256 finalizationId = finalizeRequest(tokenId);
        {
            ...
            assertEq(
                finalization.redemptionRate, newRedemptionRate, "Finalization redemption rate should match new rate"
            );
        }
        ...
        // Calculate expected amounts
        (uint256 expectedNetEthAmount, uint256 expectedFeeAmount) =
            calculateNetEthAndFee(initialAmount, newRedemptionRate, feePercentage);

        // Verify that the user received the correct amount based on the new (lower) redemption rate
        assertEq(
            user.balance - userBalanceBefore,
            expectedNetEthAmount,
            "User should receive ETH based on new (lower) redemption rate"
        );
    ...
    function testclaimWithdrawalWithComputedFinalizationId(uint256 _amount) public {
        vm.assume(_amount > 0 && _amount < 10_000 ether);
        ...
        assertEq(user.balance, expectedNetEthAmount, "testclaimWithdrawals: E5");
        assertEq(
            redeemableAsset.balanceOf(address(manager)),
            (extraWithdrawalsBefore + extraWithdrawalsAfter) * 1 ether,
            "testclaimWithdrawals: E6"
        );
        assertEq(feeReceiver.balance, expectedFeeAmount, "testclaimWithdrawals: E7");
        assertApproxEqAbs(
            address(redemptionAssetsVault).balance, _vaultBalanceBefore - _amount, 1000, "testclaimWithdrawals: E8"
    ...
    function testWithdrawSurplusRedemptionAssets() public {
        ...
        assertEq(
            address(redeemableAsset).balance - _ynethBalanceBefore, amount, "testWithdrawSurplusRedemptionAssets: E2"
        );
    }
    ...
    function testWithdrawSurplusRedemptionAssetsAmountExceedsSurplus() public {
        ...
        vm.deal(address(redemptionAssetsVault), amount);
        assertEq(
            manager.surplusRedemptionAssets(), amount, "testWithdrawSurplusRedemptionAssetsAmountExceedsSurplus: E0"
        );
        ...
        vm.prank(redemptionAssetWithdrawer);
        vm.expectRevert(
            abi.encodeWithSelector(WithdrawalQueueManager.AmountExceedsSurplus.selector, amountExceedingSurplus, amount)
        );
        manager.withdrawSurplusRedemptionAssets(amountExceedingSurplus);

        assertEq(
            manager.surplusRedemptionAssets(), amount, "testWithdrawSurplusRedemptionAssetsAmountExceedsSurplus: E1"
        );
        assertEq(
            address(redeemableAsset).balance - _ynethBalanceBefore,
            0,
            "testWithdrawSurplusRedemptionAssetsAmountExceedsSurplus: E2"
        );
    }
    ...
    function testWithdrawalRequest() public {
        ...
        assertEq(
            request.redemptionRateAtRequestTime, redemptionAssetsVault.redemptionRate(), "testWithdrawalRequest: E3"
        );
    ...
    function testSetWithdrawalFee(uint256 _feePercentage) public {
        ...
        assertEq(manager.withdrawalFee(), 10_000, "testSetWithdrawalFee: E0"); // from setUp
    ...
    function testSetWithdrawalFeeWrongCaller() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AccessControlUnauthorizedAccount.selector, address(this), manager.WITHDRAWAL_QUEUE_ADMIN_ROLE()
            )
        );
        manager.setWithdrawalFee(_feePercentage);
    }
    ...
    function testFinalizeRequestsUpToIndexSuccessfullyForMultipleRequests(
        uint256 _amount,
        uint256 requestIncrease
    )
        public
    {
        vm.assume(_amount > 0 && _amount < 10_000 ether);
    ...
    function testFinalizeRequestsUpToIndexWrongCaller() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AccessControlUnauthorizedAccount.selector, address(this), manager.REQUEST_FINALIZER_ROLE()
            )
        );
        manager.finalizeRequestsUpToIndex(requestIndex);
    }
    ...
    function testFinalizeRequestsUpToIndexWithInvalidIndex() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                WithdrawalQueueManager.IndexExceedsTokenCount.selector, requestIndex, manager._tokenIdCounter()
            )
        );
        vm.prank(requestFinalizer);
        manager.finalizeRequestsUpToIndex(requestIndex);
    ...
    function testFinalizeRequestsNotAdvanced() public {
        ...
        uint256 requestIndex = initialFinalizedIndex; // Same as the last finalized index to trigger IndexNotAdvanced
            // error

        vm.expectRevert(
            abi.encodeWithSelector(
                WithdrawalQueueManager.IndexNotAdvanced.selector, requestIndex, initialFinalizedIndex
            )
        );
        vm.prank(requestFinalizer);
        manager.finalizeRequestsUpToIndex(requestIndex);
    ...
    function testFinalizeRequestsUpToIndexWithPreExistingRequest() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(WithdrawalQueueManager.IndexNotAdvanced.selector, requestIndex, requestIndex)
        );
        vm.prank(requestFinalizer);
        manager.finalizeRequestsUpToIndex(requestIndex);
    }
    ...
    function testFindFinalizationForTokenId() public {
        ...
        for (uint256 i = 0; i < 12; i++) {
            ...
            assertEq(
                actualFinalizationId,
                expectedFinalizationIds[i],
                string(abi.encodePacked("Incorrect finalization ID for token ", vm.toString(tokenIds[i])))
            );
        }
    ...
    function testPauseWrongCaller() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                AccessControlUnauthorizedAccount.selector, address(this), redemptionAssetsVault.PAUSER_ROLE()
            )
        );
        redemptionAssetsVault.pause();
    }
    ...
    function testUnpauseWrongCaller() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                AccessControlUnauthorizedAccount.selector, address(this), redemptionAssetsVault.UNPAUSER_ROLE()
            )
        );
        redemptionAssetsVault.unpause();
    }
    ...
    function testWithdrawalRequestsForOwner() public {
        ...
        (uint256[] memory withdrawalIndexes, IWithdrawalQueueManager.WithdrawalRequest[] memory requests) =
            manager.withdrawalRequestsForOwner(user);
